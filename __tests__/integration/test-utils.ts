...
export function runCoreActionTests(actions: Action[]) {
  const formattedNames = actions.map(action => action.name);
  ...
  const composedExamples = actions.flatMap(action => 
...
export function documentTestResult(testName: string, result: any) {
  console.log(`Test Result for ${testName}:`, JSON.stringify(result, null, 2));
}